...
const withAvailable = (reagent: Reagent) => ({ ...reagent, availableStock: reagent.stock - reagent.frozenStock });

export const reagentService = {
  list(lowOnly = false) {
    const items = lowOnly ? reagents.filter((item) => item.stock - item.frozenStock < item.minStock) : reagents;
    return items.map(withAvailable);
  },
  detail(id: string) {
    const reagent = reagents.find((item) => item.id === id);
    if (!reagent) throw new ApiError(404, "REAGENT_NOT_FOUND", "试剂不存在");
    return { ...withAvailable(reagent), usageHistory: reagentUsages.filter((usage) => usage.reagentId === id) };
  },
